# KlipDrop

Live Website 🚀  
https://klipdrop.vercel.app/

Before running the website a quick hack☠️
Wake up the backend using this link🔗 -- 
https://clipdrop-lio8.onrender.com/get/test

KlipDrop is a clipboard sharing web app that lets you paste text on one device and retrieve it on another using a unique code or QR scan.

## Features
- Generate unique clipboard code
- QR code sharing
- Retrieve text from any device
- Global access using cloud backend

## Tech Stack
Frontend: HTML, CSS, JavaScript  
Backend: Node.js + Express  
Database: MongoDB Atlas  
Hosting: Render + Vercel
